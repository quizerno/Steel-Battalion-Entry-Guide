...
//Steel Battalion Home Build Input Configuration for ogx360_t4
//Provided by Quizerno
//This build will provide the settings for a MIDDLE BLOCK represented by a full length keyboard
...
void steelbattalion_task(uint8_t type_index, KeyboardController *kb, MouseController *m, JoystickController *joy)
{
    ...
    if (xid_send_report_ready(index))
    {
      ...
      //Leftovers from keyboard configuration
    ...
    //Digital Gear Shifter
        if (is_key_pressed(KEY_TAB, 1))         sb_data.gearLever -= (sb_data.gearLever > 7) ? 1 : 0;
        if (is_key_pressed(KEY_LEFTSHIFT, 1))         sb_data.gearLever += (sb_data.gearLever < 13) ? 1 : 0;


            if (is_key_pressed(KEY_E, 1))    sb_data.tunerDial += (sb_data.tunerDial < 15) ? 1 : -15;
            if (is_key_pressed(KEY_Q, 1))  sb_data.tunerDial -= (sb_data.tunerDial > 0) ? 1 : -15;
      ...
      //Maintenance Buttons
        if (is_key_pressed(KEY_I, 0))                sb_data.dButtons[1] |= CXBX_SBC_GAMEPAD_W1_WEAPONCONMAIN; //
        if (is_key_pressed(KEY_O, 0))           sb_data.dButtons[1] |= CXBX_SBC_GAMEPAD_W1_WEAPONCONSUB; //
        if (is_key_pressed(KEY_P, 0))         sb_data.dButtons[1] |= CXBX_SBC_GAMEPAD_W1_WEAPONCONMAGAZINE; //
        if (is_key_pressed(KEY_9, 0))         sb_data.dButtons[1] |= CXBX_SBC_GAMEPAD_W1_EXTINGUISHER; //
        if (is_key_pressed(KEY_0, 0))       sb_data.dButtons[1] |= CXBX_SBC_GAMEPAD_W1_CHAFF; //
        if (is_key_pressed(KEY_8, 0))         sb_data.dButtons[1] |= CXBX_SBC_GAMEPAD_W1_WASHING; //
      //Function Buttons
        if (is_key_pressed(KEY_KP7, 0))        sb_data.dButtons[1] |= CXBX_SBC_GAMEPAD_W1_FUNCTIONF1;//
        if (is_key_pressed(KEY_KP4, 0))        sb_data.dButtons[1] |= CXBX_SBC_GAMEPAD_W1_FUNCTIONF2;//
        if (is_key_pressed(KEY_KP1, 0))        sb_data.dButtons[1] |= CXBX_SBC_GAMEPAD_W1_FUNCTIONF3; //
        if (is_key_pressed(KEY_KP8, 0))        sb_data.dButtons[1] |= CXBX_SBC_GAMEPAD_W1_FUNCTIONTANKDETACH; //
        if (is_key_pressed(KEY_KP5, 0))        sb_data.dButtons[1] |= CXBX_SBC_GAMEPAD_W1_FUNCTIONOVERRIDE; //
        if (is_key_pressed(KEY_KP2, 0))        sb_data.dButtons[1] |= CXBX_SBC_GAMEPAD_W1_FUNCTIONNIGHTSCOPE; //
        if (is_key_pressed(KEY_KP9, 0))        sb_data.dButtons[0] |= CXBX_SBC_GAMEPAD_W0_FUNCTIONFSS; //
        if (is_key_pressed(KEY_KP6, 0))        sb_data.dButtons[0] |= CXBX_SBC_GAMEPAD_W0_FUNCTIONMANIPULATOR; //
        if (is_key_pressed(KEY_KP3, 0))        sb_data.dButtons[0] |= CXBX_SBC_GAMEPAD_W0_FUNCTIONLINECOLORCHANGE;//
//move modifier

            if (is_key_pressed(KEY_F1, 1))     sb_data.dButtons[2] ^= CXBX_SBC_GAMEPAD_W2_TOGGLEOXYGENSUPPLY;
            if (is_key_pressed(KEY_F2, 1))     sb_data.dButtons[2] ^= CXBX_SBC_GAMEPAD_W2_TOGGLEFILTERCONTROL;
            if (is_key_pressed(KEY_F3, 1))     sb_data.dButtons[2] ^= CXBX_SBC_GAMEPAD_W2_TOGGLEFUELFLOWRATE;
            if (is_key_pressed(KEY_F4, 1))     sb_data.dButtons[2] ^= CXBX_SBC_GAMEPAD_W2_TOGGLEBUFFREMATERIAL;
            if (is_key_pressed(KEY_F5, 1))     sb_data.dButtons[2] ^= CXBX_SBC_GAMEPAD_W2_TOGGLEVTLOCATION;
        ...
        if (is_key_pressed(KEY_PAUSE, 0))       sb_data.dButtons[0] |= CXBX_SBC_GAMEPAD_W0_EJECT;
        if (is_key_pressed(KEY_KPMINUS, 0))         sb_data.dButtons[0] |= CXBX_SBC_GAMEPAD_W0_COCKPITHATCH;
        if (is_key_pressed(KEY_KPPLUS, 0)) sb_data.dButtons[0] |= CXBX_SBC_GAMEPAD_W0_IGNITION;
        if (is_key_pressed(KEY_KPENTER, 0))     sb_data.dButtons[0] |= CXBX_SBC_GAMEPAD_W0_START;

        if (is_key_pressed(KEY_DELETE, 0))         sb_data.dButtons[0] |= CXBX_SBC_GAMEPAD_W0_MULTIMONOPENCLOSE;
        if (is_key_pressed(KEY_PAGEDOWN, 0))     sb_data.dButtons[0] |= CXBX_SBC_GAMEPAD_W0_MAINMONZOOMOUT;
        if (is_key_pressed(KEY_PAGEUP, 0))    sb_data.dButtons[0] |= CXBX_SBC_GAMEPAD_W0_MAINMONZOOMIN;
        if (is_key_pressed(KEY_HOME, 0))         sb_data.dButtons[0] |= CXBX_SBC_GAMEPAD_W0_MULTIMONMODESELECT;
        if (is_key_pressed(KEY_END, 0))         sb_data.dButtons[0] |= CXBX_SBC_GAMEPAD_W0_MULTIMONSUBMONITOR;
        if (is_key_pressed(KEY_INSERT, 0))         sb_data.dButtons[0] |= CXBX_SBC_GAMEPAD_W0_MULTIMONMAPZOOMINOUT;
     ...
     if (is_key_pressed(KEY_CAPS_LOCK, 0))
      ...
      //Only runs this if mouse is available
        if (m->available())
        {
            int32_t x, y;
